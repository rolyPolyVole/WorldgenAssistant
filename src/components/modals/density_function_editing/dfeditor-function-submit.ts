...
import { DatabaseJSONObject, DensityFunction, EventList, Modal, StringKeyObject } from "../../../types/types";
    ...
    data.database.updateEntry(
        interaction.user.id,
        {
            density_function: {
                ...
                location: entry.density_function.location.length > 0 ? entry.density_function.location + ".type" : "type"
            }
        }
    );
}
